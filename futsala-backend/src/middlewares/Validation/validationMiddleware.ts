...
export const validateBookingCreate = [
  ...
  body("startTime").matches(/^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/).withMessage("Start time must be in HH:MM format"),
  body("endTime").matches(/^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/).withMessage("End time must be in HH:MM format"),
  body("totalPrice").isFloat({ min: 0 }).withMessage("Total price must be a positive number"),
  validateRequest
];

export const validateBookingStatus = [
  body("status").optional().isIn(["PENDING", "CONFIRMED", "CANCELLED", "COMPLETED"]).withMessage("Invalid booking status"),
...
export const validateTimeSlotSchedule = [
  ...
  body("daySchedules.*.dayOfWeek").isInt({ min: 0, max: 6 }).withMessage("dayOfWeek must be between 0 (Sunday) and 6 (Saturday)"),
  body("daySchedules.*.openTime").matches(/^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/).withMessage("openTime must be in HH:MM format"),
  body("daySchedules.*.closeTime").matches(/^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/).withMessage("closeTime must be in HH:MM format"),
  body("daySchedules.*.blockedSlots").isArray().withMessage("blockedSlots must be an array of time strings"),
  validateRequest
];
